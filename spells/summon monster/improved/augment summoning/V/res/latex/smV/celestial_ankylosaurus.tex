% --- Ally Info ---
\def \name{Celestial Dinosaur, Ankylosaurus}
\def \racialtype{animal}
\def \racialsize{Huge \hspace{1pt}}
\def \racialspace{15 ft.}
\def \racialreach{15 ft.}
\def \baselandspeed{30}

\def \allyroles{Debuffer, Sensor, Smiter, Striker, Tank}
...
\attack{\tail}{\baseattack}{\strengthmod}{0}{\sizeattackmodifier}{\featattackbonus}
...
% --- Feats ---
\def \feats{Great Fortitude, Improved Bull Rush, Improved Overrun, Power Attack, Weapon Focus (tail), Weapon Proficiency (Natural)}
...
\textbf{DR} 5/evil; \textbf{SR} 17; \textbf{Resist} acid 10, cold 10, electricity 10 \\
...
\textbf{Melee} tail \enhancedstat{+\tail} \hspace{1pt} (3d6+\enhancedstat{\strengthnhalf} \hspace{1pt} plus stun) \\
...
\textbf{Stun (Ex)} The ankylosaurus's tail can deliver a powerful, stunning blow. A creature struck by this attack must make a DC \enhancedstat{25} \hspace{1pt} Fortitude save or be dazed for 1 round. If the strike is a critical hit and the target fails its save, it is instead stunned for 1d4 rounds. The save DC is Strength-based. \\
